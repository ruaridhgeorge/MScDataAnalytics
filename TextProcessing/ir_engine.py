...
# -w weighting scheme, -h help, -s stop list, -p stemming, -o output file
# From Retrieve we call index (line 85-96) query (line 158) and termWeighting (line)
...
class IndexLoader:
    def __init__(self, indexFile):
        self.index = {}
        docidCountRE = re.compile('(\d+):(\d+)') # one or more digits : one or more digits
        f = open(indexFile, 'r')
        for line in f:
            term = line.split(' ', 1)[0] # first word of each line
            self.index[term] = {}
            for (docid, count) in docidCountRE.findall(line):
                docid = int(docid)
                self.index[term][docid] = int(count) # two level dictionary {word -> {doc id -> count}
...
if __name__ == '__main__':
    ...
    retrieve = Retrieve(index, config.termWeighting) #interaction with my_retriever
    queries = Queries(config.queriesFile)
    allResults = ResultStore(config.outfile)

    for qid in queries.qids():
        query = queries.getQuery(qid)
        results = retrieve.forQuery(query) #interaction with my_retriever
